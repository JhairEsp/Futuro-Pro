...
const StudentDashboard: React.FC = () => {
  ...
  const skillsData = useMemo(() => {
    const categories = [
      { name: 'Lógica/Mates', ids: ['math'] },
      { name: 'Comunicación', ids: ['lang'] },
      { name: 'Ciencias', ids: ['sci'] },
      { name: 'Historia', ids: ['hist'] },
      { name: 'Arte', ids: ['art'] },
    ];

    return categories.map(cat => {
      const catGrades = studentGrades.filter(g => cat.ids.includes(g.courseId));
      const avg = catGrades.length > 0 
        ? catGrades.reduce((acc, curr) => acc + curr.score, 0) / catGrades.length 
        : Math.floor(Math.random() * 5) + 12; // Placeholder if no grades
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <section className="bg-gradient-to-r from-indigo-600 to-indigo-800 rounded-3xl p-8 text-white shadow-xl">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            ...
            <p className="text-indigo-100 opacity-90">Sigue así, estás en el top 10% de tu clase.</p>
          </div>
          <div className="flex items-center gap-4 bg-white/10 backdrop-blur-md p-4 rounded-2xl border border-white/20 min-w-[280px]">
            ...
            <div className="flex-1">
              <div className="flex justify-between text-sm font-bold mb-1">
                ...
                <span>{student.points} / {student.levelRank * 1000} XP</span>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatsCard title="Promedio General" value={avgGrade} icon={TrendingUp} color="blue" trend="+0.5 vs mes pasado" />
        <StatsCard title="Puntos Acumulados" value={student.points} icon={Star} color="orange" trend="120 hoy" />
        <StatsCard title="Cursos Aprobados" value="5/5" icon={BookOpen} color="green" />
        <StatsCard title="Logros Obtenidos" value={student.badges.length || 3} icon={Trophy} color="purple" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        >
          ...
          <div className="mt-4 p-4 bg-slate-50 rounded-2xl border border-slate-100">
            <p className="text-sm text-slate-600 italic">
              "Tu análisis muestra una fuerte inclinación hacia el razonamiento lógico y la resolución de problemas técnicos."
            </p>
          </div>
        </motion.div>

        {/* Bimester Performance */}
        <motion.div 
          initial={{ opacity: 0, scale: 0.95 }}
          animate={{ opacity: 1, scale: 1 }}
          className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm"
        >
          <div className="flex items-center gap-2 mb-6">
            ...
            <h3 className="text-xl font-bold text-slate-800">Evolución Académica</h3>
          </div>
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={bimesterData}>
                ...
                <Bar dataKey="score" radius={[8, 8, 0, 0]}>
                  {bimesterData.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={entry.score >= 14 ? '#10b981' : '#f43f5e'} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ...
          <div className="mt-4 flex gap-4 overflow-x-auto pb-2">
            {['Iniciador', 'Constante', 'Destacado'].map((badge) => (
              <div key={badge} className="flex-shrink-0 flex items-center gap-2 bg-indigo-50 text-indigo-700 px-4 py-2 rounded-full text-sm font-medium border border-indigo-100">
                <Award size={16} />
                {badge}
              </div>
            ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {careerRecommendations.map((career) => (
              <div key={career} className="p-4 rounded-2xl bg-orange-50 border border-orange-100 flex flex-col items-center text-center">
                ...
                <p className="text-xs text-slate-500">95% de afinidad</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h3 className="text-xl font-bold text-slate-800">LogrosRecientes</h3>
          </div>
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="font-bold text-slate-800 text-sm">Perfecto 20</p>
                <p className="text-xs text-slate-500">Obtenido en Matemáticas</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="font-bold text-slate-800 text-sm">Racha de 7 días</p>
                <p className="text-xs text-slate-500">Actividad constante</p>
              </div>
            </div>
          </div>
        </div>
